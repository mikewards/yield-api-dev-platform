...
import com.flow.dto.*
import com.flow.integration.ProtocolService
import com.flow.model.YieldAccounts
...
class YieldService {
    ...
    private val marketService = com.flow.service.MorphoMarketService()
    ...
    fun deposit(accountId: UUID, yieldAccountId: UUID, request: DepositRequest): TransactionResponse {
        return transaction {
            ...
            // Create transaction
            val now = java.time.Instant.now()
            val transactionId = com.flow.model.Transactions.insert {
                it[com.flow.model.Transactions.accountId] = accountId
                it[com.flow.model.Transactions.yieldAccountId] = yieldAccountId
                it[com.flow.model.Transactions.type] = "deposit"
                it[com.flow.model.Transactions.status] = "pending"
                it[com.flow.model.Transactions.amount] = amount
                it[com.flow.model.Transactions.currency] = request.currency
                it[com.flow.model.Transactions.sourceAddress] = request.source_address
                it[com.flow.model.Transactions.createdAt] = now
            } get com.flow.model.Transactions.id
            ...
            com.flow.model.Transactions.update({ com.flow.model.Transactions.id eq transactionId }) {
                it[com.flow.model.Transactions.status] = "completed"
                it[com.flow.model.Transactions.completedAt] = java.time.Instant.now()
            }
            ...
            TransactionResponse(
                ...
                status = "completed",
                created_at = com.flow.model.Transactions.select { 
                    com.flow.model.Transactions.id eq transactionId 
                }.first()[com.flow.model.Transactions.createdAt]?.toString() ?: now.toString()
            )
        }
    }
    ...
    fun withdraw(accountId: UUID, yieldAccountId: UUID, request: WithdrawRequest): TransactionResponse {
        return transaction {
            ...
            val transactionId = com.flow.model.Transactions.insert {
                it[com.flow.model.Transactions.accountId] = accountId
                it[com.flow.model.Transactions.yieldAccountId] = yieldAccountId
                it[com.flow.model.Transactions.type] = "withdraw"
                it[com.flow.model.Transactions.status] = "pending"
                it[com.flow.model.Transactions.amount] = amount
                it[com.flow.model.Transactions.currency] = request.currency
                it[com.flow.model.Transactions.destinationAddress] = request.destination_address
                it[com.flow.model.Transactions.createdAt] = now
            } get com.flow.model.Transactions.id
